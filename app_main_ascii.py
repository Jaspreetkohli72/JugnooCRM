...
# 1. SETUP & CONNECTION
# ---------------------------
st.set_page_config(page_title="Jugnoo", page_icon="???????", layout="wide")
...
tab1, tab2, tab3, tab5, tab6, tab4 = st.tabs(["???? Dashboard", "??? New Client", "???? Estimator", "???? Suppliers", "???? P&L", "?????? Settings"])
...
with tab3:
    ...
    if tn:
        ...
        if st.session_state[ssk]:
            ...
            if missing_items:
                st.error("?????? **Low Stock Warning:** You do not have enough inventory for: " + ", ".join(missing_items) + ". Please visit Suppliers tab to restock.")
