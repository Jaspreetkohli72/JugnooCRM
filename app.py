...
# CSS: Targets ONLY the metric cards + Login Form Styling
st.markdown("""
    <style>
...
def check_login(username, password):
    try:
        # Simple plaintext check as requested (can be upgraded to hash later)
...
def login_page():
    ...
    if st.session_state.logged_in:
        return  # Already logged in, skip to main app
        ...
        if selected_client_name:
            ...
            with c2:
                ...
                try:
                    ...
                    current_index = 0
                new_status = st.selectbox("Update Status", status_options, index=current_index, key=f"status_{client['id']}")
            ...
            if client.get('internal_estimate'):
                ...
                if isinstance(est_data, dict):
                    ...
                    if margins:
                        st.caption(f"Custom Margins: P {margins['p']*100:.0f}% | L {margins['l']*100:.0f}% | E {margins['e']*100:.0f}%")
                else:
...
# --- TAB 4: SETTINGS (USER MANAGEMENT ADDED) ---
with tab4:
    ...
    with st.form("change_pwd"):
        ...
        if st.form_submit_button("Update Profile"):
            ...
            if new_key: updates["recovery_key"] = new_key
