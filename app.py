...
# CSS: Targets metric cards & Login Form Styling
st.markdown("""
    <style>
...
def verify_recovery(username, key):
    try:
        res = supabase.table("users").select("*").eq("username", username).eq("recovery_key", key).execute()
        return len(res.data) > 0
    except:
        return False
    ...
    with c2:
        if st.session_state.auth_mode == "login":
            ...
            with st.form("recovery_form"):
                ...
                r_key = st.text_input("Recovery Key (Security Answer)")
                new_pwd = st.text_input("New Password", type="password")
                
                if st.form_submit_button("Reset Password"):
                    if verify_recovery(r_user, r_key):
                        supabase.table("users").update({"password": new_pwd}).eq("username", r_user).execute()
...
with tab1:
    ...
    if response and response.data:
        ...
        if selected_client_name:
            ...
            with c2:
                st.info(f"**Status:** {client.get('status', 'Unknown')}")
                status_options = ["Estimate Given", "Order Received", "Work In Progress", "Work Done", "Closed"]
                curr_idx = 0
                if client.get('status') in status_options:
                    curr_idx = status_options.index(client.get('status'))
                
                new_status = st.selectbox("Update Status", status_options, index=curr_idx, key=f"status_{client['id']}")
            ...
            if client.get('internal_estimate'):
                ...
                if isinstance(est_data, dict):
                    ...
                    if margins:
                        st.caption(f"Margins: P {margins['p']*100:.0f}% | L {margins['l']*100:.0f}% | E {margins['e']*100:.0f}%")
                else:
...
with tab3:
    ...
    if target_client_name:
        ...
        if st.session_state[ss_key]:
            ...
            c1.metric("Material Cost", f"₹{mat_total:,.0f}")
            c2.metric("Labor Cost", f"₹{labor_client:,.0f}")
            ...
            pdf_bytes = create_pdf(target_client_name, current_items, labor_client, days_to_complete, grand_total)
...
with tab4:
    ...
    with st.form("margin_settings"):
        ...
        p = c1.slider("Part %", 0, 100, p_curr)
        l = c2.slider("Labor %", 0, 100, l_curr)
        e = c3.slider("Extra %", 0, 100, e_curr)
    ...
    with st.form("change_pwd"):
        ...
        new_key = st.text_input("Update Recovery Key (Optional)", help="Used for Forgot Password")
        
        if st.form_submit_button("Update Profile"):
            updates = {"password": new_p}
            if new_key:
                updates["recovery_key"] = new_key
            
            run_query(supabase.table("users").update(updates).eq("username", st.session_state.username))
