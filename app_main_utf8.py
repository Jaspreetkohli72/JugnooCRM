...
# 1. SETUP & CONNECTION
# ---------------------------
st.set_page_config(page_title="Jugnoo", page_icon="≡ƒÅù∩╕Å", layout="wide")
...
top_c1.write(f"≡ƒæñ Logged in as: **{st.session_state.username}**")
...
tab1, tab2, tab3, tab5, tab6, tab4 = st.tabs(["≡ƒôï Dashboard", "Γ₧ò New Client", "≡ƒº« Estimator", "≡ƒÜÜ Suppliers", "≡ƒôê P&L", "ΓÜÖ∩╕Å Settings"])
...
with tab1:
    ...
    with st.spinner("Loading Dashboard..."):
        ...
        if response and response.data:
            ...
            with st.expander("≡ƒ¢á∩╕Å Manage Client"):
                ...
                if sel_name:
                    ...
                    st.markdown("### ≡ƒ¢á∩╕Å Manage Client")
                    ...
                    with c1:
                        ...
                        with st.form("edit_details"):
                            ...
                            if client.get('location'):
                                st.link_button("≡ƒÜÇ Open Location in Maps", url=client['location'], use_container_width=True)
    
                            if st.form_submit_button("≡ƒÆ╛ Save Changes"):
                                try:
                    ...
                    with c2:
                        ...
                        if n_stat in ["Order Received", "Work In Progress", "Work Done"]:
                            ...
                            s_date = st.date_input("≡ƒôà Start Date", value=def_d)
                        if st.button("Update Status", key=f"btn_{client['id']}"):
                            ...
                            if n_stat == "Closed":
                                st.write("**≡ƒÆ░ Record Payment Received**")
                                ...
                                if est_data:
                                    ...
                                    with payment_col1:
                                        st.metric("Grand Total", f"Γé╣{est_advance:,.0f}")
                                    with payment_col2:
                                        actual_payment = st.number_input("Amount Received (Γé╣)", min_value=0.0, value=float(est_advance), step=100.0, key=f"payment_{client['id']}")
                    ...
                    if client.get('internal_estimate'):
                        st.divider()
                        st.subheader("≡ƒôä Manage Estimate")
                        ...
                        if st.session_state[ssk_dash]:
                            ...
                            edited_est = st.data_editor(idf, num_rows="dynamic", use_container_width=True, key=f"de_{client['id']}",
                                                        column_config={
                                                            ...
                                                            "Unit Price": st.column_config.NumberColumn("Unit Price", format="Γé╣%.2f", width="small", disabled=True),
                                                            "Total Price": st.column_config.NumberColumn("Total Price", format="Γé╣%.2f", width="small", disabled=True)
                                                        })
                            ...
                            m1.metric("Material Total", f"Γé╣{mat_sell:,.0f}"); m2.metric("Labor", f"Γé╣{labor_charged_display:,.0f}"); m3.metric("Grand Total", f"Γé╣{rounded_grand_total:,.0f}"); m4.metric("Total Profit", f"Γé╣{total_profit:,.0f}"); m5.metric("Advance Required", f"Γé╣{advance_amount:,.0f}")
                            
                            if st.button("≡ƒÆ╛ Save Changes", key=f"sv_{client['id']}"):
                                ...
                                try:
                                    ...
                                    st.toast("Saved!", icon="Γ£à")
                                except Exception as e:
                                    st.error(f"Database Error: {e}")
                            
                            st.write("#### ≡ƒôÑ Download Bills")
                            ...
                            c_pdf1.download_button("≡ƒôä Client Invoice", pdf_client, f"Invoice_{client['name']}.pdf", "application/pdf", key=f"pdf_c_{client['id']}")
                            ...
                            if client.get('status') == "Work Done":
                                ...
                                st.metric("Net Profit (from Grand Total)", f"Γé╣{total_profit:,.0f}")
                            else: 
...
with tab3:
    ...
    if tn:
        ...
        with col1:
            uc = st.checkbox("≡ƒ¢á∩╕Å Use Custom Margins", value=(sm is not None), key="cm")
        with col2:
            dys = st.number_input("ΓÅ│ Days", min_value=1, step=1, value=int(sd))
        am = gs
        if uc:
        ...
        if inv and inv.data:
            ...
            with st.form("add_est"):
                ...
                if st.form_submit_button("Γ¼ç∩╕Å Add"):
        ...
        if st.session_state[ssk]:
            ...
            if missing_items:
                st.error("ΓÜá∩╕Å **Low Stock Warning:** You do not have enough inventory for: " + ", ".join(missing_items) + ". Please visit Suppliers tab to restock.")
            ...
            c1.metric("Material", f"Γé╣{mt:,.0f}"); c2.metric("Labor", f"Γé╣{disp_lt:,.0f}"); c3.metric("Grand Total", f"Γé╣{rounded_gt:,.0f}"); c4.metric("Total Profit", f"Γé╣{total_profit:,.0f}"); c5.metric("Advance Required", f"Γé╣{advance_amount:,.0f}")
            
            cs, cp = st.columns(2)
            if cs.button("≡ƒÆ╛ Save", type="primary"):
                df_to_save = edf.copy()
                for col in ['Qty', 'Base Rate', 'Total Price', "Unit Price"]:
                    df_to_save[col] = pd.to_numeric(df_to_save[col].fillna(0))
                for col in ['Item', 'Unit']: df_to_save[col] = df_to_save[col].fillna("")
                ...
                try:
                    ...
                    if res and res.data: st.toast("Saved!", icon="Γ£à")
                except Exception as e:
                    st.error(f"Database Error: {e}")
            
            pbytes = create_pdf(tc['name'], edf.to_dict(orient="records"), dys, disp_lt, rounded_gt, advance_amount)
            cp.download_button("≡ƒôä Download PDF", pbytes, f"Est_{tc['name']}.pdf", "application/pdf", key=f"pe_{tc['id']}")
...
with tab4:
    ...
    with st.form("glob_set"):
        ...
        lc = st.number_input("Daily Labor (Γé╣)", value=float(s.get('daily_labor_cost', 1000.0)), step=100.0)
    ...
    st.divider()
    st.subheader("≡ƒÆ░ Advance Payment Calculator")
    
    st.write("**How Advance is Calculated:**")
    st.info("""
    Advance Required = (Total Base Cost + (Total Profit ├ù Advance Percentage)) / 100 ├ù 100 (rounded)
    ...
    with st.form("advance_calc_settings"):
        ...
        with col_ex1:
            ...
            st.text(f"Base Cost: Γé╣{example_base_cost:,}")
            st.text(f"Profit: Γé╣{example_profit:,}")
            st.text(f"Advance ({advance_pct}%): Γé╣{example_advance:,}")
        ...
        with col_ex2:
            ...
            st.text(f"Daily Labor Cost: Γé╣{float(s.get('daily_labor_cost', 1000)):,.0f}")
    ...
    if inv_resp and inv_resp.data:
        ...
        if st.button("≡ƒÆ╛ Save Inventory Changes"):
...
# --- TAB 5: SUPPLIERS ---
with tab5:
    st.header("≡ƒÜÜ Supplier & Purchase Management")
    ...
    if supplier_resp and supplier_resp.data:
        ...
        if st.button("≡ƒÆ╛ Save Supplier Changes", key="save_sup_changes"):
        ...
        with col_purchase:
            ...
            with st.form("record_purchase_form"):
                ...
                if supplier_options and inventory_options:
                    ...
                    if st.form_submit_button("Γ£à Record Transaction", type="primary"):
...
# --- TAB 6: PROFIT & LOSS ---
with tab6:
    st.subheader("≡ƒôê Profit & Loss Analysis")
    ...
    try:
        ...
        st.write("#### ≡ƒôè Key Business Metrics")
        kpi_col1, kpi_col2, kpi_col3, kpi_col4 = st.columns(4)
        kpi_col1.metric("Completed Projects", completed_projects)
        kpi_col2.metric("Total Revenue (Grand Total)", f"Γé╣{total_revenue:,.0f}")
        kpi_col3.metric("Actual Payment Received", f"Γé╣{total_payment_received:,.0f}", delta=f"Γé╣{total_payment_received - total_revenue:,.0f}")
        kpi_col4.metric("Total Expenses", f"Γé╣{total_expenses:,.0f}")

        # === KPI #2: Profit Metrics ===
        st.divider()
        st.write("#### ≡ƒÆ░ Profit Analysis")
        profit_col1, profit_col2, profit_col3, profit_col4 = st.columns(4)
        profit_col1.metric("Estimated Profit", f"Γé╣{net_profit_estimated:,.0f}")
        profit_col2.metric("Actual Profit (After Discounts)", f"Γé╣{actual_profit:,.0f}", delta=f"Γé╣{actual_profit - net_profit_estimated:,.0f}")
        profit_col3.metric("Margin %", f"{actual_margin_pct:.1f}%")
        profit_col4.metric("Discount Loss", f"Γé╣{total_revenue - total_payment_received:,.0f}")

        # === KPI #3: Expense Breakdown ===
        st.divider()
        st.write("#### ≡ƒôê Expense Breakdown")
        ...
        expense_col1.metric("Material Expenses", f"Γé╣{total_material_expense:,.0f}", delta=f"{material_pct:.1f}%")
        expense_col2.metric("Labor Expenses", f"Γé╣{total_labor_expense:,.0f}", delta=f"{labor_pct:.1f}%")
        expense_col3.metric("Avg Cost per Project", f"Γé╣{total_expenses / completed_projects:,.0f}" if completed_projects > 0 else "N/A")

        st.divider()
        st.write("#### ≡ƒôè Financial Overview")
        ...
        with chart_col1:
            ...
            comparison_chart = alt.Chart(chart_data_comparison).mark_bar().encode(
                ...
                y=alt.Y('Amount:Q', axis=alt.Axis(title="Amount (Γé╣)", format='.0f')),
                ...
                tooltip=[
                    ...
                    alt.Tooltip('Amount:Q', format=',.0f', title='Amount (Γé╣)')
                ]
        ...
        st.divider()
        st.write("#### ≡ƒôè Additional Analytics")
        ...
        with analytics_col1:
            ...
            profit_chart = alt.Chart(chart_data_profit).mark_bar().encode(
                ...
                y=alt.Y('Amount:Q', axis=alt.Axis(title="Profit (Γé╣)", format='.0f')),
        ...
        with analytics_col2:
            ...
            st.text(f"Γ£ô You capture {(total_payment_received / total_revenue * 100) if total_revenue > 0 else 0:.1f}% of quoted amount")
            st.text(f"Γ£ô Operating margin: {actual_margin_pct:.1f}%")
            st.text(f"Γ£ô {completed_projects} projects completed")
